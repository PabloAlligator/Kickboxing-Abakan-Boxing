...
// Этап 1: минимальный набор функций для клуба.
// Остальные render-функции ниже намеренно сохранены для следующих этапов.
...
const primaryMobileRoutes = new Set(['dashboard', 'athletes', 'personal']);
...
const rubles = (cents = 0) =>
  `${new Intl.NumberFormat('ru-RU').format(Math.round(cents / 100))} ₽`;
...
const empty = (titleText, text) =>
  `<div class="empty-state"><h3>${escapeHtml(titleText)}</h3><p>${escapeHtml(text)}</p></div>`;
...
function setLoading() {
  app.innerHTML =
    '<div class="loading-state"><span class="spinner"></span><p>Загрузка данных</p></div>';
}
...
function groupOptions(selected = '') {
  return `<option value="">Выберите группу</option>${state.groups
    .filter((group) => group.active)
    .map(
      (group) =>
        `<option value="${group.id}" ${String(selected) === String(group.id) ? 'selected' : ''}>${escapeHtml(group.name)}</option>`,
    )
    .join('')}`;
}
...
function categoryOptions(selected = '') {
  return state.categories
    .map(
      (category) =>
        `<option value="${category.id}" ${String(selected) === String(category.id) ? 'selected' : ''}>${escapeHtml(category.name)}</option>`,
    )
    .join('');
}
...
async function renderDashboard() {
  ...
  app.innerHTML = `
    <div class="section-head"><div><h2>Состояние клуба</h2><p>Спортсмены и фактические групповые оплаты</p></div>${monthControl()}</div>
    <section class="stats-grid">
      ${stat('Активные спортсмены', data.athletes)}
      ${stat('Оплатили за месяц', data.paidAthletes)}
      ${stat('Не оплатили', data.unpaidAthletes)}
      ${stat('Получено за месяц', rubles(data.income), true)}
    </section>
    <section class="content-grid content-grid--two">
      <div class="panel">
        <div class="panel-head"><h3>Последние оплаты</h3><a href="/admin/athletes" class="button button--secondary">Спортсмены</a></div>
        ${data.latestPayments.length ? `<div class="card-list">${data.latestPayments.map((item) => card(item.athlete.fullName, `${shortDate(item.paymentDate)} · ${rubles(item.amountCents)}`)).join('')}</div>` : empty('Оплат пока нет', 'Отметить оплату можно прямо в карточке спортсмена.')}
      </div>
      <div class="panel">
        <div class="panel-head"><h3>Быстрый доступ</h3></div>
        <div class="card-list">
          ${card('Спортсмены', 'Добавление спортсменов и контроль оплат', '<a class="button button--primary" href="/admin/athletes">Открыть</a>')}
          ${card('Мои персоналки', 'Личные клиенты, тренировки и доход', '<a class="button button--secondary" href="/admin/personal">Открыть</a>')}
        </div>
      </div>
    </section>`;
}
...
async function renderAthletes() {
  const groupsData = await api('/api/admin/groups');
  state.groups = groupsData.groups;
  const data = await api(
    `/api/admin/athletes?status=${state.athleteStatus}&month=${state.month}${state.athletePayment ? `&payment=${state.athletePayment}` : ''}`,
  );
  state.athletes = data.athletes;
  app.innerHTML = `
    <div class="section-head"><div><h2>Спортсмены</h2><p>${data.athletes.length} в выбранном списке</p></div><div class="button-group"><button class="button button--secondary" data-action="manage-groups">Группы</button><button class="button button--primary" data-action="new-athlete">Добавить спортсмена</button></div></div>
...
function athleteCard(athlete) {
  const groups =
    athlete.memberships.map((item) => item.group.name).join(', ') ||
    'Без группы';
  const paymentAction =
    athlete.status === 'ACTIVE'
      ? `<button class="button button--primary" data-action="pay-athlete" data-id="${athlete.id}" data-name="${escapeHtml(athlete.fullName)}">${athlete.paidCents ? 'Добавить оплату' : 'Отметить оплату'}</button>`
      : '';
  const statusActions =
    athlete.status === 'ACTIVE'
      ? `<button class="button button--danger" data-action="archive-athlete" data-id="${athlete.id}">В архив</button>`
      : `<button class="button button--primary" data-action="restore-athlete" data-id="${athlete.id}">Вернуть</button><button class="button button--danger" data-action="delete-athlete" data-id="${athlete.id}" data-name="${escapeHtml(athlete.fullName)}">Удалить навсегда</button>`;
  ...
  return `<article class="data-card" data-athlete-card data-name="${escapeHtml(athlete.fullName.toLowerCase())}" data-groups="${athlete.memberships.map((item) => item.group.id).join(',')}">
    ...
    <div class="button-group">${paymentAction}<button class="button button--secondary" data-action="edit-athlete" data-id="${athlete.id}">Карточка</button>${statusActions}</div>
  </article>`;
}
...
function groupUpdateBody(group, changes = {}) {
  return {
    name: changes.name ?? group.name,
    startTime: group.startTime || '00:00',
    days: Array.isArray(group.days) ? group.days : [],
    coachIds: (group.coaches || []).map((item) => item.user.id),
    showPublic: group.showPublic === true,
...
async function openGroupsManager() {
  ...
  showModal(
    'Группы спортсменов',
    `
    <p class="muted">Группы сейчас используются только для распределения спортсменов. Расписание и тренеров можно будет включить позже.</p>

    <form class="stack-form" data-form="quick-group">
      <label>
        Новая группа
        <input name="name" required maxlength="120" placeholder="Например: Старшая группа">
      </label>
      <button class="button button--primary" type="submit">Создать группу</button>
    </form>
          ...
          <article class="data-card">
            <div class="data-card__head">
              <div>
                ...
                <p>Спортсменов: ${group.memberships?.length || 0}</p>
              </div>
              <span class="badge ${group.active ? 'badge--green' : ''}">${group.active ? 'Активна' : 'Архив'}</span>
            </div>
            <div class="button-group">
              <button class="button button--secondary" type="button" data-action="rename-quick-group" data-id="${group.id}">Переименовать</button>
            </div>
          </article>
        `,
              )
          ...
          : empty(
              'Групп пока нет',
              'Создайте первую группу, чтобы распределять спортсменов.',
            )
      }
    </div>
  `,
...
function quickGroupRenameForm(group) {
  return `
    <form class="stack-form" data-form="quick-group-edit" data-id="${group.id}">
      <label>
        Название группы
        <input name="name" required maxlength="120" value="${escapeHtml(group.name)}">
      </label>
      <div class="button-group">
        <button class="button button--secondary" type="button" data-action="manage-groups">Назад</button>
...
async function renderAttendance() {
  await loadReferences(true);
  let details = null;
  if (state.attendanceTrainingId)
    details = await api(
      `/api/admin/trainings/${state.attendanceTrainingId}/attendance`,
    );
...
async function renderCalendar() {
  ...
  const now = new Date();
  let from = monthStart;
  let toDate = new Date(`${monthStart}T00:00:00Z`);
...
async function renderTasks() {
  const [active, completed] = await Promise.all([
    api('/api/admin/tasks'),
    api('/api/admin/tasks?completed=true'),
  ]);
...
async function route() {
  const requested =
    ...
    'dashboard';
  state.route =
    ENABLED_ROUTES.has(requested) && pageNames[requested]
      ? requested
      : 'dashboard';

  // Скрываем подготовленные, но пока не выданные клиенту разделы.
  document.querySelectorAll('[data-route]').forEach((link) => {
    if (!ENABLED_ROUTES.has(link.dataset.route)) link.style.display = 'none';
  });

  title.textContent = pageNames[state.route];
  document
    .querySelectorAll('.admin-sidebar__link[data-route]')
    .forEach((link) => {
  ...
  document
    .querySelectorAll('.admin-bottom-nav__link[data-route]')
    .forEach((link) => {
      const active = link.dataset.route === state.route;
  ...
  mobileMenu?.classList.toggle(
    'admin-bottom-nav__more--active',
    !primaryMobileRoutes.has(state.route),
  );
  closeSidebar();
  setLoading();
  try {
...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'manage-groups') return openGroupsManager();
    if (action === 'rename-quick-group') {
      const group = state.groups.find(
        (item) => item.id === Number(actionNode.dataset.id),
      );
      if (!group) throw new Error('Группа не найдена');
      return showModal('Переименовать группу', quickGroupRenameForm(group));
    }
    if (action === 'new-athlete')
      return showModal('Новый спортсмен', athleteForm());
    if (action === 'edit-athlete') {
      const { athlete } = await api(
        `/api/admin/athletes/${actionNode.dataset.id}`,
      );
      return showModal('Карточка спортсмена', athleteForm(athlete));
    }
    if (
      action === 'archive-athlete' &&
      confirm('Перенести спортсмена в архив?')
    ) {
      await api(`/api/admin/athletes/${actionNode.dataset.id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status: 'ARCHIVED' }),
      });
      toast('Спортсмен архивирован');
      return route();
    }
    if (action === 'restore-athlete') {
      await api(`/api/admin/athletes/${actionNode.dataset.id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status: 'ACTIVE' }),
      });
      toast('Спортсмен возвращён');
      return route();
    }
    if (action === 'delete-athlete') {
      const athleteName = actionNode.dataset.name || 'этого спортсмена';
      const confirmed = confirm(
        `Удалить спортсмена «${athleteName}» навсегда?\n\n` +
          'Будут удалены:\n' +
          '— карточка спортсмена\n' +
          '— все его оплаты\n' +
          '— история посещений\n' +
      ...
      if (!confirmed) return;
      await api(`/api/admin/athletes/${actionNode.dataset.id}`, {
        method: 'DELETE',
      });
      toast('Спортсмен удалён навсегда');
      return route();
    }
    ...
    if (action === 'edit-group') {
      ...
      return showModal('Настройки группы', groupForm(group));
    }
    if (action === 'add-group-member')
      return showModal(
        'Добавить в группу',
        `<form class="stack-form" data-form="group-member" data-id="${actionNode.dataset.id}"><label>Спортсмен<select name="athleteId" required>${athleteOptions()}</select></label><button class="button button--primary" type="submit">Добавить</button></form>`,
      );
    if (action === 'attendance-all' || action === 'attendance-none') {
      document
        .querySelectorAll('[data-form="attendance"] input[type="checkbox"]')
        .forEach((input) => {
          input.checked = action === 'attendance-all';
        });
      return;
    }
    if (action === 'pay-athlete') {
      const athleteId = Number(actionNode.dataset.id);
      const athleteName = actionNode.dataset.name || 'Спортсмен';
      return showModal(
        'Отметить оплату',
        `<form class="stack-form" data-form="payment"><input type="hidden" name="athleteId" value="${athleteId}"><p class="muted">${escapeHtml(athleteName)}</p><div class="form-row"><label>Дата<input type="date" name="paymentDate" required value="${isoDate()}"></label><label>Сумма, ₽<input type="number" name="amount" min="1" step="1" required></label></div><label>Комментарий<textarea name="comment" maxlength="1000"></textarea></label><button class="button button--primary" type="submit">Сохранить оплату</button></form>`,
      );
    }
    // Полноценный раздел оплат пока скрыт, но обработчик оставлен для следующего этапа.
    if (action === 'new-payment')
      return showModal(
        'Добавить оплату',
        `<form class="stack-form" data-form="payment"><label>Спортсмен<select name="athleteId" required>${athleteOptions()}</select></label><div class="form-row"><label>Дата<input type="date" name="paymentDate" required value="${isoDate()}"></label><label>Сумма, ₽<input type="number" name="amount" min="1" step="1" required></label></div><label>Комментарий<textarea name="comment"></textarea></label><button class="button button--primary" type="submit">Сохранить оплату</button></form>`,
      );
    if (
      action === 'delete-payment' &&
      confirm('Удалить платёж? Финансовая история изменится.')
    ) {
      await api(`/api/admin/payments/${actionNode.dataset.id}`, {
        method: 'DELETE',
      });
      toast('Платёж удалён');
      return route();
    }
    if (action === 'new-personal-client')
      return showModal(
        'Персональный клиент',
        `<form class="stack-form" data-form="personal-client"><label>Имя<input name="name" required maxlength="160"></label><label>Заметки<textarea name="notes" maxlength="3000"></textarea></label><button class="button button--primary" type="submit">Создать карточку</button></form>`,
      );
    if (action === 'new-personal-training') {
      const { clients } = await api('/api/admin/personal/clients');
      return showModal(
        'Персональная тренировка',
        `<form class="stack-form" data-form="personal-training"><label>Клиент<select name="clientId" required><option value="">Выберите клиента</option>${clients.map((client) => `<option value="${client.id}">${escapeHtml(client.name)}</option>`).join('')}</select></label><div class="form-row"><label>Дата и время<input type="datetime-local" name="startsAt" required></label><label>Цена, ₽<input type="number" name="price" min="1" required></label></div><label>Напоминание<select name="reminderMinutes"><option value="1440">За день</option><option value="120">За 2 часа</option><option value="">Без напоминания</option></select></label><label>Комментарий<textarea name="comment" maxlength="3000"></textarea></label><button class="button button--primary" type="submit">Запланировать</button></form>`,
      );
    }
    if (action === 'view-personal-client') {
      const { client } = await api(
        `/api/admin/personal/clients/${actionNode.dataset.id}`,
      );
      return showModal(
        client.name,
        `<p class="muted">${escapeHtml(client.notes || 'Без общих заметок')}</p><div class="card-list">${client.trainings.length ? client.trainings.map((item) => card(dateTime(item.startsAt), `${statusName(item.status)} · ${rubles(item.priceCents)}${item.comment ? ` · ${item.comment}` : ''}`)).join('') : empty('История пуста', 'Тренировок ещё нет.')}</div>`,
      );
    }
    if (
      action === 'view-personal-training' ||
      action === 'edit-personal-training'
    ) {
      ...
      return showModal(
        'Персональная тренировка',
        `<form class="stack-form" data-form="personal-training-edit" data-id="${training.id}">
    ...
    if (action === 'complete-personal' || action === 'cancel-personal') {
      await api(`/api/admin/personal/trainings/${actionNode.dataset.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: action === 'complete-personal' ? 'COMPLETED' : 'CANCELLED',
        }),
      });
      toast('Статус тренировки обновлён');
      return route();
    }
    if (action === 'new-task')
      return showModal(
        'Новая задача',
        `<form class="stack-form" data-form="task"><label>Название<input name="title" required maxlength="240"></label><div class="form-row"><label>Срок<input type="datetime-local" name="dueAt"></label><label>Напоминание<input type="datetime-local" name="reminderAt"></label></div><label>Приоритет<select name="priority"><option value="NORMAL">Обычный</option><option value="IMPORTANT">Важный</option></select></label><label>Комментарий<textarea name="comment"></textarea></label><button class="button button--primary" type="submit">Создать задачу</button></form>`,
      );
    if (['complete-task', 'restore-task'].includes(action)) {
      await api(`/api/admin/tasks/${actionNode.dataset.id}`, {
        method: 'PUT',
        body: JSON.stringify({ completed: action === 'complete-task' }),
      });
      toast('Задача обновлена');
      return route();
    }
    if (action === 'delete-task' && confirm('Удалить задачу?')) {
      await api(`/api/admin/tasks/${actionNode.dataset.id}`, {
        method: 'DELETE',
      });
    ...
    if (action === 'new-category')
      return showModal(
        'Новая категория',
        `<form class="stack-form" data-form="category"><label>Название<input name="name" required maxlength="100"></label><button class="button button--primary" type="submit">Добавить</button></form>`,
      );
    if (action === 'new-expense')
      return showModal(
        'Добавить расход',
        `<form class="stack-form" data-form="expense"><label>Название<input name="title" required maxlength="240"></label><div class="form-row"><label>Дата<input type="date" name="expenseDate" value="${isoDate()}" required></label><label>Сумма, ₽<input type="number" name="amount" min="1" required></label></div><label>Категория<select name="categoryId" required>${categoryOptions()}</select></label><label>Комментарий<textarea name="comment"></textarea></label><button class="button button--primary" type="submit">Сохранить расход</button></form>`,
      );
    if (
      action === 'delete-expense' &&
      confirm('Удалить расход? Общая статистика изменится.')
    ) {
      await api(`/api/admin/expenses/${actionNode.dataset.id}`, {
        method: 'DELETE',
      });
      toast('Расход удалён');
      return route();
    ...
    if (action === 'calendar-mode') {
      state.calendarMode = actionNode.dataset.mode;
...
document.addEventListener('submit', async (event) => {
  ...
  try {
    if (type === 'quick-group') {
      ...
      state.groups = [];
      return openGroupsManager();
    } else if (type === 'quick-group-edit') {
      const group = state.groups.find(
        (item) => item.id === Number(form.dataset.id),
      );
      if (!group) throw new Error('Группа не найдена');
      await api(`/api/admin/groups/${group.id}`, {
        method: 'PUT',
        body: JSON.stringify(groupUpdateBody(group, { name: values.name })),
      });
      ...
      await api(
        form.dataset.id
          ? `/api/admin/athletes/${form.dataset.id}`
          : '/api/admin/athletes',
        {
      ...
      const body = {
        ...
        coachIds: data.getAll('coachIds').map(Number),
        showPublic: data.has('showPublic'),
        active: form.dataset.id ? data.has('active') : true,
      };
      await api(
        form.dataset.id
          ? `/api/admin/groups/${form.dataset.id}`
          : '/api/admin/groups',
        {
          method: form.dataset.id ? 'PUT' : 'POST',
          body: JSON.stringify(body),
      ...
      await api(`/api/admin/groups/${form.dataset.id}/members`, {
        method: 'POST',
        body: JSON.stringify(values),
      });
    } else if (type === 'open-attendance') {
      const trainings = await api(
        `/api/admin/trainings?from=${values.trainingDate}&to=${values.trainingDate}&groupId=${values.groupId}`,
      );
      let training = trainings.trainings[0];
      if (!training)
        training = (
          await api('/api/admin/trainings', {
            method: 'POST',
            body: JSON.stringify(values),
          })
        ).training;
      state.attendanceTrainingId = training.id;
      return renderAttendance();
    } else if (type === 'attendance') {
      const checked = new Set(new FormData(form).getAll('athlete').map(Number));
      const attendance = [
        ...form.querySelectorAll('input[name="athlete"]'),
      ].map((input) => ({
        athleteId: Number(input.value),
        present: checked.has(Number(input.value)),
      }));
      await api(`/api/admin/trainings/${form.dataset.id}/attendance`, {
        method: 'PUT',
      ...
      await api('/api/admin/personal/clients', {
        method: 'POST',
        body: JSON.stringify(values),
      });
    } else if (type === 'personal-training') {
      await api('/api/admin/personal/trainings', {
        method: 'POST',
        body: JSON.stringify({
          ...values,
      ...
      await api(`/api/admin/personal/trainings/${form.dataset.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          startsAt: new Date(values.startsAt).toISOString(),
          priceCents: toCents(values.price),
...
document.addEventListener('change', (event) => {
  if (event.target.matches('[data-action="change-month"]')) {
    state.month = event.target.value;
    route();
  }
  if (event.target.matches('[data-athlete-status]')) {
    state.athleteStatus = event.target.value;
    renderAthletes();
  }
  if (event.target.matches('[data-athlete-payment]')) {
    state.athletePayment = event.target.value;
    renderAthletes();
  }
...
function filterAthletes() {
  const search =
    document
      .querySelector('[data-filter="athlete-search"]')
      ?.value.toLowerCase() || '';
  const group =
    document.querySelector('[data-filter="athlete-group"]')?.value || '';
  document.querySelectorAll('[data-athlete-card]').forEach((cardNode) => {
    cardNode.hidden =
      !cardNode.dataset.name.includes(search) ||
      (group && !cardNode.dataset.groups.split(',').includes(group));
  });
}
...
async function enablePush() {
  ...
  if (!status.configured || !status.publicKey) {
    throw new Error('Web Push ещё не настроен на сервере');
  }
  ...
  // Удаляем старую Push-подписку, если она была создана
  // с предыдущим VAPID applicationServerKey.
...
(async function init() {
  try {
    ...
    if (Number.isInteger(trainingId) && trainingId > 0)
      state.attendanceTrainingId = trainingId;
