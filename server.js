...
function validateProductionEnv() {
  ...
  const required = ['DATABASE_URL', 'SESSION_SECRET', 'APP_ORIGIN', 'PUBLIC_PHONE', 'TELEGRAM_URL', 'VAPID_PUBLIC_KEY', 'VAPID_PRIVATE_KEY', 'VAPID_SUBJECT'];
  const missing = required.filter((key) => !String(process.env[key] || '').trim());
  if (missing.length) throw new Error(`Отсутствуют production ENV: ${missing.join(', ')}`);
  ...
  try {
    ...
    if (origin.protocol !== 'https:' || origin.origin !== String(process.env.APP_ORIGIN)) {
      throw new Error();
    }
  } catch {
    throw new Error('APP_ORIGIN в production должен быть HTTPS origin без пути и завершающего слеша');
  }
  ...
  try {
    ...
    if (telegram.protocol !== 'https:' || !['t.me', 'telegram.me'].includes(telegram.hostname.toLowerCase())) {
      throw new Error();
    }
  } catch {
...
function createApp() {
  ...
  const app = express();
  const sessionSecret = process.env.SESSION_SECRET || 'development-only-secret-change-before-production-0123456789abcdef0123456789abcdef';
  const sessionStore = process.env.NODE_ENV === 'test' ? undefined : new PrismaSessionStore();
  ...
  app.use('/api/admin', requireAuth.api, requireCsrf, (req, res, next) => {
  ...
  app.get('/admin', requireAuth.page, (_req, res) => res.redirect(302, '/admin/dashboard'));
  ...
  app.use('/site/css', express.static(path.join(SITE_DIR, 'css'), staticOptions));
  app.use('/site/scripts', express.static(path.join(SITE_DIR, 'scripts'), staticOptions));
  app.use('/site/img', express.static(path.join(SITE_DIR, 'img'), staticOptions));
  app.use('/site/fonts', express.static(path.join(SITE_DIR, 'fonts'), staticOptions));
  ...
  app.use((_req, res) => res.status(404).type('text/plain').send('Страница не найдена'));
...
function start() {
  ...
  const reminderTimer = setInterval(() => {
    runReminderCycle().catch((error) => console.error('Планировщик:', error.message));
  }, 60 * 1000);
  reminderTimer.unref();

  const sessionCleanupTimer = sessionStore ? setInterval(() => {
    sessionStore.clearExpired().catch((error) => console.error('Очистка сессий:', error.message));
  }, 60 * 60 * 1000) : null;
